...
The Command Center dashboard is always at `http://localhost:3000`. If it's not responding, remind the user to run `~/.command-center/start.sh`.
...
If the server isn't running: `~/.command-center/start.sh`
...
### `@lu standup`
Read today's daily log from `~/.command-center/daily/YYYY-MM-DD.md` (use today's actual date).
...
Write to `~/.command-center/daily/YYYY-MM-DD.md`:
```markdown
## YYYY-MM-DD
