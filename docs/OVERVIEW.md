...
It runs entirely on your machine. No cloud required. All data lives in a local SQLite DB at `~/.command-center/cc.db`.
...
This is a **local-first tool**. SQLite requires zero infrastructure — no connection strings, no Docker, no cloud accounts. The entire app starts with `npm run dev`. The DB file at `~/.command-center/cc.db` travels with the developer.
...
9. If a git worktree was created for this role (when the project has a local repo path), it exists at `~/.command-center/worktrees/`
...
- Includes artifact count, status, ticket link, and a "View in Command Center" button
