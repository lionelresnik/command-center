...
// ─── DB Setup ────────────────────────────────────────────────────────────────
const DB_PATH = path.join(os.homedir(), ".command-center", "cc.db");
function getDb() {
    if (!fs.existsSync(DB_PATH)) {
        throw new Error(`Command Center database not found at ${DB_PATH}.\n` +
            `Make sure the Command Center app has been run at least once.`);
    }
    return new better_sqlite3_1.default(DB_PATH, { readonly: true });
}
// ─── Helpers ─────────────────────────────────────────────────────────────────
...
// ─── Tool: cc_status ─────────────────────────────────────────────────────────
server.tool("cc_status", "Get a quick overview of Command Center: active missions, open todos, recent activity, and project list.", {}, async () => {
    const db = getDb();
    try {
...
// ─── Tool: cc_list_missions ───────────────────────────────────────────────────
server.tool("cc_list_missions", "List missions, optionally filtered by status or project name.", {
...
// ─── Tool: cc_get_mission ─────────────────────────────────────────────────────
server.tool("cc_get_mission", "Get full details of a specific mission including task graph, artifacts, and questions.", {
...
// ─── Tool: cc_list_todos ──────────────────────────────────────────────────────
server.tool("cc_list_todos", "List todos, optionally filtered by status or priority.", {
...
// ─── Tool: cc_add_todo ────────────────────────────────────────────────────────
server.tool("cc_add_todo", "Add a new todo item to Command Center.", {
...
// ─── Tool: cc_complete_todo ───────────────────────────────────────────────────
server.tool("cc_complete_todo", "Mark a todo as done.", {
...
// ─── Tool: cc_search_knowledge ────────────────────────────────────────────────
server.tool("cc_search_knowledge", "Search the knowledge base by keyword. Returns matching entries with full content.", {
...
// ─── Tool: cc_add_knowledge ───────────────────────────────────────────────────
server.tool("cc_add_knowledge", "Add a new entry to the knowledge base.", {
...
// ─── Tool: cc_list_projects ───────────────────────────────────────────────────
server.tool("cc_list_projects", "List all projects with their IDs, descriptions, and integration settings.", {}, async () => {
    const db = getDb();
    try {
        const projects = db.prepare(`SELECT * FROM projects ORDER BY name`).all();
        if (projects.length === 0) {
            return { content: [{ type: "text", text: "No projects found. Create one in the Command Center UI." }] };
        }
...
// ─── Tool: cc_get_project_context ─────────────────────────────────────────────
server.tool("cc_get_project_context", "Get full context for a project: knowledge base, active missions, and open todos. Use this to give an AI full context before working on a task.", {
    projectId: zod_1.z.string().describe("Project ID (use cc_list_projects to find it)"),
    ...
    try {
        ...
        if (!project) {
            return { content: [{ type: "text", text: `Project \`${projectId}\` not found. Use cc_list_projects to see available projects.` }] };
        }
        const sections = [
    ...
    try {
      ...
      `).run(missionId, missionName, goal, projectRow?.id ?? null, workspaceRow?.id ?? null, JSON.stringify(projectIds), crewRow.id, ticketId ?? null, behavior, JSON.stringify(taskGraph));
        const lines = [
            ...
            ``,
            `To run: use cc_run_mission with id: \`${missionId}\``,
...
// ─── Tool: cc_run_mission ─────────────────────────────────────────────────────
server.tool("cc_run_mission", "Trigger the next role in a mission. Calls the Command Center API to run the next pending agent. The web app must be running (npm run dev). Returns a summary when the role finishes.", {
    id: zod_1.z.string().describe("Mission ID"),
    appUrl: zod_1.z.string().optional().describe("Command Center URL. Defaults to http://localhost:3000"),
    ...
    let missionName = id;
    try {
        const m = db.prepare(`SELECT name, status FROM missions WHERE id = ?`).get(id);
        if (!m)
            return { content: [{ type: "text", text: `Mission \`${id}\` not found.` }] };
        if (m.status === "done")
    ...
    try {
        ...
        return {
            content: [{
                    type: "text",
                    text: `Could not reach Command Center at ${appUrl}.\nMake sure the app is running: cd command-center/app && npm run dev\n\nError: ${err}`,
                }]
        };
    }
    const lines = [
        ...
        result.nextRole ? `**Next role:** ${result.nextRole} — run cc_run_mission again to continue` : "",
        result.tokensUsed ? `**Tokens used:** ${result.tokensUsed.toLocaleString()}` : "",
        result.questions?.length > 0
            ? `\n⚠ **Agent has questions:**\n${result.questions.map((q) => `- ${q}`).join("\n")}\nUse cc_get_questions to answer them.`
            : "",
        ``,
...
// ─── Tool: cc_get_questions ───────────────────────────────────────────────────
server.tool("cc_get_questions", "Get unanswered questions from agents on a mission. Use cc_answer_question to respond.", {
      ...
      `;
        ...
        const lines = [
            ...
            ...rows.map(q => [
                ...
                `To answer: use cc_answer_question with id: "${q.id}"`,
            ].join("\n")),
        ];
        return { content: [{ type: "text", text: lines.join("\n\n") }] };
    }
...
// ─── Tool: cc_answer_question ─────────────────────────────────────────────────
server.tool("cc_answer_question", "Answer a question from an agent. The answer is saved to the DB and used as context when the next role runs.", {
    id: zod_1.z.string().describe("Question ID (from cc_get_questions)"),
...
// ─── Tool: cc_open ────────────────────────────────────────────────────────────
server.tool("cc_open", "Open or register the current repo as a Command Center project. Detects git remote automatically. Creates the project if it doesn't exist, optionally assigns it to a workspace.", {
...
server.tool("cc_list_workspaces", "List all workspaces with their projects.", {}, async () => {
    const db = getDb();
    try {
...
server.tool("cc_create_workspace", "Create a new workspace and optionally assign existing projects to it.", {
...
// ─── Tool: cc_export ─────────────────────────────────────────────────────────
server.tool("cc_export", "Export Command Center data as clean markdown — ready to paste into Claude.ai, ChatGPT, or any AI chat. Exports knowledge base, missions, and todos for a project (or everything).", {
...
// ─── Tool: cc_import_v1 ───────────────────────────────────────────────────────
server.tool("cc_import_v1", "Import data from v1 Command Center files (~/.command-center/todos.md, daily-log/*.jsonl, task-history/**/*.md) into the v2 database. Skips duplicates.", {
    ...
    const CC_DIR = path.join(os.homedir(), ".command-center");
    ...
    try {
        ...
        if (what.includes("task_history")) {
            ...
            if (!fs.existsSync(histDir)) {
                ...
                report.push("## Task History\n⚠ `projectId` is required to import task history. Use cc_list_projects to find your project ID.\n");
            }
            else {
...
async function main() {
    ...
    process.stderr.write("Command Center MCP server running\n");
}
main().catch(err => {
