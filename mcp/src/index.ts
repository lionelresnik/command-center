...
function getDb(): Database.Database {
  if (!fs.existsSync(DB_PATH)) {
    throw new Error(
      `Command Center database not found at ${DB_PATH}.\n` +
...
server.tool(
  "cc_status",
  "Get a quick overview of Command Center: active missions, open todos, recent activity, and project list.",
  {},
  async () => {
    const db = getDb()
    try {
...
server.tool(
  "cc_add_todo",
  "Add a new todo item to Command Center.",
  {
...
server.tool(
  ...
  {
    projectId: z.string().describe("Project ID (use cc_list_projects to find it)"),
...
server.tool(
  "cc_create_mission",
  "Create a new mission in Command Center. Finds project and crew by name, builds the task graph, and saves to DB.",
  {
    goal: z.string().describe("What the mission should accomplish"),
    project: z.string().describe("Project name (partial match OK)"),
  ...
  async ({ goal, project, crew, name, behavior = "assume_and_document", ticketId }) => {
    ...
    try {
      // Find project
      const projectRow = db.prepare(`SELECT id, name FROM projects WHERE name LIKE ? LIMIT 1`).get(`%${project}%`) as { id: string; name: string } | undefined
      if (!projectRow) {
...
server.tool(
  "cc_run_mission",
  "Trigger the next role in a mission. Calls the Command Center API to run the next pending agent. The web app must be running (npm run dev). Returns a summary when the role finishes.",
  {
    id: z.string().describe("Mission ID"),
...
server.tool(
  "cc_get_questions",
  "Get unanswered questions from agents on a mission. Use cc_answer_question to respond.",
  {
...
server.tool(
  ...
  {
    id: z.string().describe("Question ID (from cc_get_questions)"),
...
server.tool(
  "cc_export",
  "Export Command Center data as clean markdown — ready to paste into Claude.ai, ChatGPT, or any AI chat. Exports knowledge base, missions, and todos for a project (or everything).",
  {
