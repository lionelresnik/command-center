...
echo ""
echo "  ⚡ Command Center v2 — Installer"
echo "  ─────────────────────────────────"
echo ""
...
#!/usr/bin/env bash
APP_DIR="$(dirname "$(dirname "$(realpath "$0")")")/Projects/command-center-v2/app"

# Try to find the app directory
if [ ! -d "$APP_DIR" ]; then
  # Fallback: look for it relative to common locations
  for candidate in ~/Projects/command-center-v2/app ~/code/command-center-v2/app ~/dev/command-center-v2/app; do
    if [ -d "$candidate" ]; then
      APP_DIR="$candidate"
      break
    fi
...
if [ ! -d "$APP_DIR" ]; then
  echo "[cc] Could not find command-center-v2/app. Update start.sh with the correct path."
  exit 1
fi

cd "$APP_DIR"
echo "⚡ Starting Command Center v2..."
...
echo ""
echo "  ✅ Command Center v2 installed!"
echo ""
echo "  Start the dashboard:   ~/.command-center/start.sh"
