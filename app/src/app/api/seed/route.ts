...
const { projects, knowledgeEntries, missions, roles, teams, settings } = schema
...
4. Keep docs concise and accurate`,
    ...
    await db.insert(projects).values([
      { id: p1id, name: "Platform API", description: "Core backend platform and API layer", color: "#3b82f6", githubRepo: "platform-api", githubOwner: "my-org", jiraProject: "PLAT", agentsMdStatus: "merged" },
      { id: p2id, name: "Auth Service", description: "Authentication and authorization service", color: "#10b981", githubRepo: "auth-service", githubOwner: "my-org", jiraProject: "AUTH", agentsMdStatus: "pr_open" },
      { id: p3id, name: "Billing", description: "Billing and subscription management", color: "#f59e0b", githubRepo: "billing", githubOwner: "my-org", jiraProject: "BILL", agentsMdStatus: "local" },
    ])

    // ── Missions ──────────────────────────────────────────────────────────────
    const m1id = nanoid(), m2id = nanoid()
    ...
    return NextResponse.json({ ok: true, message: "Seeded: 5 roles, 2 crews, 3 projects, 2 missions, 5 knowledge entries — using generic demo data" })
  } catch (e) {
